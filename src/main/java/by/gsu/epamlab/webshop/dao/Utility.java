...
public class Utility {
    ...
    public static int getTotalPages(ProductDaoImpl entity, int rows) {
        int totalPages = entity.getNumberOfRecords() / rows;
        if (entity.getNumberOfRecords() % rows > 0){
            totalPages +=1;
        }
            return totalPages;
    }
    ...
    public static Product createProductFromString(String stringProduct){
        int idProduct ;
        String productName ;
        Byn price ;
        String describe ;
        String[] array = stringProduct.split(";");
        if (array.length == 4) {
        ...
        return new Product(idProduct,productName,price,describe);
    }
